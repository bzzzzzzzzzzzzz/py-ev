...
# Decryption by Bob
    ...
    def evaluation_arguments(capsule, cfrag, pieces):
            return bytes(capsule), bytes(cfrag), b''.join(pieces)
    ...
    account = accounts[0] 
    # Deploy contract 
    deploy_contract = ReEncryptionValidator.deploy({"from":account}) 
    # Print contract address 
    print(f"contract deployed at {deploy_contract}")

    transaction_receipt = deploy_contract.validateCFrag(eval_args[0], eval_args[1], eval_args[2])
